...
import Pyro4
# Configuración de PyRO4: serializador consistente entre daemon, nameserver y proxies
Pyro4.config.SERIALIZER           = "serpent"
Pyro4.config.SERIALIZERS_ACCEPTED = {"serpent"}
Pyro4.config.HOST                 = "localhost"  # ajusta a IP real si usas varias máquinas
...
# Definimos los métodos públicos
    ...
    def iniciar(self, distribuido: bool = False) -> None:
        for jugador in self.jugadores:
            jugador.trabajador.start()
        print(f"\n {self.nombre} abierta ")
        # Si el modo distribuido está activo, se registra la calle en PyRO4
        if distribuido:
...
class Barrio(ABC):
    # Iniciamos el constructor de clases
    def __init__(self, nombre: str, distribuido: bool = False) -> None:
        self.nombre      = nombre
        self.distribuido = distribuido
        self.calles: list[Calle] = []
        self.cerrojo = Lock()
        # En modo distribuido: nombres de calles remotas para crear Proxies
        self._nombres_calles_remotas: list[str] = []
...
# Definimos los métodos privados
    ...
    def _get_inventario_calle(self, calle_o_nombre) -> dict:
        if self.distribuido:
            # CORRECCIÓN: proxy como context manager para liberar el socket al terminar
            with obtener_proxy_calle(calle_o_nombre) as proxy:
...
# Definimos los métodos públicos
    ...
    def añadir_calle(self, calle) -> None:
        if self.distribuido:
    ...
    def iniciar(self, distribuido: bool = False) -> None:
        for calle in self.calles:
            calle.iniciar(distribuido=distribuido)
...
class BarrioComercial(Barrio):
    # Iniciamos el constructor de clases
    def __init__(self, nombre: str, intervalo: int = 15,
                 distribuido: bool = False) -> None:
        super().__init__(nombre, distribuido)
...
# Definimos los métodos privados
    ...
    def _redistribuir_recursos(self) -> None:
        ...
        if self.distribuido:
            ...
            for item in snap_a:
                ...
                if exceso_a > 0 and deficit_b > 0:
                    ...
                    if self._retirar_de_calle(nombre_a, item, transferir):
            ...
            for item in snap_b:
                ...
                if exceso_b > 0 and deficit_a > 0:
                    ...
                    if self._retirar_de_calle(nombre_b, item, transferir):
        ...
        if len(self.calles) < 2:
            return
        # CORRECCIÓN: orden de adquisición por id() para evitar deadlock
        # con el hilo _equilibrar_barrios del Pueblo que adquiere calles en el mismo orden.
        calle_a, calle_b = sorted(self.calles, key=id)
...
class BarrioEconomico(Barrio):
    ...
    """
    # Definimos el constructor de clases
    def __init__(self, nombre: str, intervalo: int = 20,
                 distribuido: bool = False) -> None:
        super().__init__(nombre, distribuido)
...
# Definimos los métodos privados
    ...
    def _ajustar_precios(self) -> None:
        #  Modo distribuido: notificamos pero no fijamos precio remotamente 
        # Los jugadores remotos actualizan sus precios desde su propio script
        if self.distribuido:
...
# SEMANA 3 — Distribución con PyRO4
# IMPORTANTE: Requiere nameserver activo antes de ejecutar
#   →  pyro4-ns  (en una terminal aparte)


#  Mixin que expone una Calle como objeto remoto PyRO4 
# Métodos mínimos: el barrio solo necesita leer inventario,
# transferir recursos y consultar info de jugadores.
# Lock y Thread NO son serializables por PyRO4, por eso
...
#  Función para registrar una Calle en el nameserver 
# Se llama desde Calle.iniciar() cuando distribuido=True.
# Levanta el daemon en un hilo daemon para no bloquear el MUD.
    ...
    def _servidor():
        ...
        with Pyro4.Daemon() as daemon:
            ...
            with Pyro4.locateNS() as ns:
                ...
                ns.register(calle.nombre, uri, safe=False)  # safe=False: permite re-registro sin reiniciar el nameserver
...
#  Función para obtener un Proxy a una calle remota
...
# CLASE PUEBLO
# Contenedor de dos barrios (uno Comercial, uno Económico).
# Gestiona oferta/demanda global entre barrios. 
class Pueblo:
    # Iniciamos el constructor de clases
    def __init__(self, nombre: str) -> None:
        self.nombre = nombre
...
# Definimos los métodos privados
    ...
    def _equilibrar_barrios(self) -> None:
        ...
        # CORRECCIÓN: adquirimos TODOS los cerrojos de ambos barrios simultáneamente
        # en orden fijo (id) para evitar deadlock circular con _redistribuir_recursos.
        # Esto elimina la ventana TOCTOU entre la fase de lectura (snapshot) y la de escritura.
        todas_las_calles = sorted(barrio_a.calles + barrio_b.calles, key=id)
        with self.cerrojo:
            for calle in todas_las_calles:
                calle.cerrojo.acquire()
            try:
                    ...
                    if exceso > 0 and deficit > 0:
                        ...
                        if ya_movido > 0:
                            calle_destino = min(barrio_b.calles,
                                               key=lambda c: c.inventario.get(item, 0))
                            calle_destino.inventario[item] = (
                                calle_destino.inventario.get(item, 0) + ya_movido
                            )
                            print(f"[Pueblo {self.nombre}] {ya_movido} {item}: "
                                  f"{barrio_a.nombre} → {barrio_b.nombre}.")
            finally:
                for calle in todas_las_calles:
                    calle.cerrojo.release()
...
# Definimos los métodos públicos
    ...
    def ranking_jugadores(self) -> None:
        ...
        for pos, (nombre, monedas, calle_n, barrio_n) in enumerate(jugadores, 1):
            print(f"  {pos}. {nombre} ({calle_n}, {barrio_n}): {monedas} monedas")



# Generamos el mud para que corra el juego (Semana 3: incluye pueblo)
...
riqueza                                → riqueza total del barrio / pueblo
...
"""
    ...
    while True:
            ...
            if len(partes) < 3:
                ...
                try:
                    ...
                    if calle_origen is None:
                        print(f"No hay suficiente '{item_c}' disponible.")
                    else:
        ...
        # Parte de las peticiones del MUD, para cambiar de jugador activo
            ...
            if len(partes) < 2:
                # Lista todos los jugadores disponibles según el contexto
                if pueblo:
...
if __name__ == "__main__":
    ...
    #  Calle de la Seda (tejido / costura) 
    recetas_hilandero = [
        {"produce": "hilo", "cantidad": 1, "necesita": {}, "tiempo": 3}
    ]
    recetas_costurero = [
        {"produce": "tela",  "cantidad": 1, "necesita": {"hilo": 2}, "tiempo": 5},
        {"produce": "ropa",  "cantidad": 1, "necesita": {"tela": 3}, "tiempo": 7},
    ]
    t_hilandero = Trabajador(recetas_hilandero, {}, Lock(), {"hilo": 2})
    t_costurero = Trabajador(recetas_costurero, {}, Lock(), {"tela": 6, "ropa": 18})
    lucia   = Jugador("Lucia",   t_hilandero, monedas=50)
    roberto = Jugador("Roberto", t_costurero, monedas=50)
    calle_seda = Calle("Calle de la Seda")
    calle_seda.añadir_jugador(lucia)
    calle_seda.añadir_jugador(roberto)


    #  Calle del Pan (agricultura / panadería) 
    recetas_agricultor = [
        {"produce": "trigo", "cantidad": 1, "necesita": {}, "tiempo": 4}
    ]
    recetas_panadero = [
        {"produce": "harina", "cantidad": 1, "necesita": {"trigo": 2}, "tiempo": 5},
        {"produce": "pan",    "cantidad": 1, "necesita": {"harina": 1}, "tiempo": 3},
    ]
    t_agricultor = Trabajador(recetas_agricultor, {}, Lock(), {"trigo": 2})
    t_panadero   = Trabajador(recetas_panadero,   {}, Lock(), {"harina": 5, "pan": 10})
    sofia  = Jugador("Sofia",  t_agricultor, monedas=50)
    miguel = Jugador("Miguel", t_panadero,   monedas=50)
    calle_pan = Calle("Calle del Pan")
    calle_pan.añadir_jugador(sofia)
    calle_pan.añadir_jugador(miguel)


    #  Barrio Comercial (madera + hierro) 
    ...
    #  Barrio Económico (seda + pan) 
    barrio_economico = BarrioEconomico("Barrio de la Plaza", intervalo=20)
    barrio_economico.añadir_calle(calle_seda)
    barrio_economico.añadir_calle(calle_pan)


    #  Pueblo 
    pueblo = Pueblo("Pueblo de Albaverde")
